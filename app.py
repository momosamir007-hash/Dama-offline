...
def inject_css():
    ...
    <style>
        ...
        .status-player { background: linear-gradient(135deg, #1B5E20, #2E7D32); color: #FFFFFF; border: 2px solid #4CAF50; }
        .status-ai { background: linear-gradient(135deg, #B71C1C, #C62828); color: #FFFFFF; border: 2px solid #EF5350; }
        .status-win { background: linear-gradient(135deg, #F9A825, #FFD54F); color: #1A1A1A; border: 2px solid #FFD700; font-size: 1.3rem; }
        .status-lose { background: linear-gradient(135deg, #424242, #616161); color: #FFFFFF; border: 2px solid #9E9E9E; }
        .status-draw { background: linear-gradient(135deg, #0D47A1, #1565C0); color: #FFFFFF; border: 2px solid #42A5F5; }
        ...
        div[data-testid="stSidebar"] { font-family: 'Tajawal', sans-serif; }
    </style>
...
def main():
    ...
    with st.sidebar:
        ...
        if new_game:
            if "أبيض" in color_choice: init_game(WHITE, BLACK, depth)
            else: init_game(BLACK, WHITE, depth)
            st.rerun()
        ...
        if st.session_state.get("game_started"):
                ...
                <div class="info-card">
                    <div class="piece-count">
                        <div class="piece-side">
                            ...
                            <div style="font-size:0.8rem">{w_men} قطعة {f'+ {w_kings} ملك' if w_kings else ''}</div>
                        </div>
                        <div style="font-size:1.5rem;color:#666">⚔️</div>
                        <div class="piece-side">
                            <div style="font-size:1.5rem">⬛</div>
                            <div class="piece-num">{b_total}</div>
                            <div style="font-size:0.8rem">{b_men} قطعة {f'+ {b_kings} ملك' if b_kings else ''}</div>
                        </div>
                    </div>
                </div>
            """, unsafe_allow_html=True)
            ...
            if st.session_state.move_history:
                ...
                history_container = st.container(height=250)
    ...
    if not st.session_state.get("game_started"):
        ...
        svg = render_board_svg(demo_board)
